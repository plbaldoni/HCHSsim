...
for(i in idx){
  ...
  m1.list.mitime[[i]] <- data.table(Sim = i, Time.Calibration = summary(time.calibration,'s')$mean, Time.Model = summary(time.model.m1,'s')$mean, Time.Resampling = summary(time.mi.m1,'s')$mean)
  m2.list.mitime[[i]] <- data.table(Sim = i, Time.Calibration = summary(time.calibration,'s')$mean, Time.Model = summary(time.model.m2,'s')$mean, Time.Resampling = summary(time.mi.m2,'s')$mean)
}
cat('Done!',"\n")
